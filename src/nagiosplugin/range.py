...
class Range(nagiosplugin.valueobj.ValueObject):
    ...
    def match(self, value):
        """Return True if `value` is inside the bounds (modulo inversion).

        If value is a non-numeric object, a ValueError is raised.
        """
        if not isinstance(value, numbers.Number):
            raise ValueError('cannot match non-numeric value', value)
